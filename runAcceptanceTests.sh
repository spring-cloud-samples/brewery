...
# CLOUD FOUNDRY -- START
export CLOUD_FOUNDRY_STACK="cflinuxfs3"
...
function deploy_app_with_name(){
    ...
    cf push ${APP_NAME} --no-start -f "manifest-${CLOUD_PREFIX}.yml" -b https://github.com/cloudfoundry/java-buildpack.git#v4.21 -s "${CLOUD_FOUNDRY_STACK}"
    APPLICATION_DOMAIN=`app_domain ${APP_NAME}`
...
[[ -z "${REPO_BRANCH}" ]] && REPO_BRANCH="2020.0.x"
...
export START_ZIPKIN=${START_ZIPKIN}
...
if [[ -z "${CLOUD_FOUNDRY}" ]] ; then
    ...
    if [[ "${WHAT_TO_TEST}" == "SCS" ]] ; then
        ...
        for i in $( seq 1 "${RETRIES}" ); do
            sleep "${WAIT_TIME}"
            CURL_RESULT=$( curl --fail -m 5 https://${DISCOVERY_HOST}/eureka/apps/ || echo "failed to reach discovery server" )
